...
public:
	...
	*/
	...
	// プレイヤーの位置に基づいて追従カメラを設定
	static void ForceCameraScroll(float playerPosX) {
		// 追従カメラを取得
		CCamera* pMainCamera = CCamera::GetCamera(CCamera::eMainCamera);
		if (pMainCamera) {
			// プレイヤーの位置に合わせてカメラの位置を更新（補間を追加）
			CVector3D currentPos = pMainCamera->GetPos();

			// 追従させるX座標の補間
			float targetX = playerPosX + 40.0f;  // プレイヤーのX位置に少しオフセットを加える
			float lerpSpeed = 0.1f;  // 補間の速度（0～1の間で調整）

			// 補間（スムーズな追従を実現）
			currentPos.x += (targetX - currentPos.x) * lerpSpeed;

			// カメラの位置と向きを更新
			pMainCamera->SetTranseRot(currentPos, pMainCamera->GetDir());
		}

		// メインカメラとして追従カメラを設定
		CCamera::SetCurrent(CCamera::eMainCamera);
	}
};